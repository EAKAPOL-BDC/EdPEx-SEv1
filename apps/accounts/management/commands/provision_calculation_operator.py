...
ROLE = 'calculation-operator-v1'

class Command(BaseCommand):
    help = 'Preview a calculation.run-only grant to an existing scoped superuser; --apply writes.'
    ...
    def handle(self, *args, **options):
        ...
        if role and role.permissions != ['calculation.run']:
            raise CommandError('Existing role differs; no changes made.')
        self.stdout.write(f'User: {user.username}; scope: {scope.pk} ({scope.name}); permission: calculation.run; descendants: no; expiry: none')
        ...
        if not options['apply']:
            ...
            return
        if role is None:
            role = Role.objects.create(code=ROLE, permissions=['calculation.run'])
        grant = RoleAssignment.objects.create(membership=member, scope=scope, role=role, include_descendants=False)
        record_event(scope.organization, user, 'calculation.operator_provisioned', 'RoleAssignment', grant.pk,
                     reason=reason, metadata={'scope_id':str(scope.pk), 'membership_id':str(member.pk),
                                             'role_id':str(role.pk), 'permissions':['calculation.run'],
                                             'authorization_channel':'local_admin_command'})
        self.stdout.write(self.style.SUCCESS('Calculation permission provisioned. Refresh the closed survey round.'))
