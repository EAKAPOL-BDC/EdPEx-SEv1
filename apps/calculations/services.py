...
def _audit(actor, collection_round, action, object_id, reason='', **metadata):
    record_event(collection_round.organization, actor, action, "calculations.calculationrun", object_id, reason=reason,
...
def _uses_f06_contract(version, contract_version):
    """A wording revision may reuse the checked scoring contract, not a new formula.

    The current binding, question types, groups, scales and numeric options are
    still validated below. The frozen definition retains the actual form version.
    Arbitrarily named or unrelated versions do not acquire compatibility.
    """
    if version.version == contract_version:
        return True
    expected_method = {'F06':'self_report','F05':'verified_activity'}.get(version.instrument.code,'survey')
    if version.instrument.code not in {'F01','F02','F03','F04','F05','F06'} or version.assessment_method != expected_method:
        return False
    seen = {version.pk}
    parent = version.based_on
    while parent is not None:
        if parent.pk in seen or parent.instrument_id != version.instrument_id or parent.assessment_method != expected_method:
            return False
        seen.add(parent.pk)
        if parent.version == contract_version:
            return True
        parent = parent.based_on
    return False
...
def _definition(collection_round, item, catalog):
    ...
    if version.instrument.code=='F05' and version.assessment_method=='self_report':
        from apps.governance.f05_registry import for_version
        contract=for_version(version.version)
        validate_version,MAP,VERSION=contract.validate_version,contract.MAP,contract.VERSION
        from .types import FormulaSpec
        validate_version(version)
        if group.code not in version.group_codes or item.dimension is not None or binding.formula.status not in {'published','retired'}:
            raise ValidationError('Invalid anonymous F05 group or formula.')
        key,qids,_=MAP[binding.indicator.code]
        spec=FormulaSpec(key,tuple(qids),VERSION,VERSION)
        series={'scope_id':str(collection_round.scope_id),'period_id':str(collection_round.period_id),'calendar_type':collection_round.period.calendar.calendar_type,
                'indicator_code':binding.indicator.code,'group_code':group.code,'context':selected.context,'dimension':None,
                'method':'anonymous_self_report','response_unit':version.response_unit,'spec':encode_snapshot_spec(spec),'privacy_mode':'anonymous-v2','denominator':contract.PARAMETERS['denominator']}
        definition={'series':series,'instrument':{'id':str(version.pk),'code':'F05','version':version.version,'checksum':version.checksum,'method':'anonymous_self_report'},
                ...
                'questions':[_question_snapshot(q) for q in binding.questions.prefetch_related('options').order_by('question_id')],
                'formula':{'id':str(binding.formula_id),'key':key,'version':VERSION,'definition_th':binding.formula.definition_th,'parameters':binding.formula.parameters}}
    ...
    if (not _uses_f06_contract(version, spec.instrument_version) or binding.formula.key != spec.formula_key
    ...
    series = {
        ...
        "spec": encode_snapshot_spec(spec),
    }
    if version.instrument.code=='F06' and hasattr(selected,'survey_profile'):
        series['privacy_mode']='anonymous-v2'
        series['method']='anonymous_self_report'
    if version.version != spec.instrument_version:
        # Separate trend/series identity while replaying the unchanged 1.1 math.
        series['instrument_version'] = version.version
    if version.instrument.code == 'F04':
        from apps.leadership.services import validate_binding
        target = validate_binding(selected, collection_round.population_snapshot)
        if target is not None:
            series['leadership'] = target.snapshot
    definition = {
        "series": series,
...
def _prepare_input(collection_round, item, cutoff, catalog):
    ...
    anonymous_staff = context.instrument_id in {'F05','F06'} and hasattr(selected,'survey_profile')
    anonymous_annual = context.instrument_id=='F05' and selected.instrument_version.assessment_method=='self_report'
    if anonymous_annual and not anonymous_staff:raise ValidationError('Anonymous F05 requires its anonymous source profile.')
    if spec.formula_key in ACTIVITY_KEYS and not anonymous_annual:
        if item.responses:
    ...
    if anonymous_staff:
        if len(rows)>group_count:raise ValidationError('Responses exceed eligible population.')
        # These are synthetic, unlinked units, never roster IDs.
        ids=[row['unit_id'] for row in rows]+['unanswered-'+str(i) for i in range(group_count-len(rows))]
        population={'snapshot_id':str(snapshot.pk),'unit_ids':ids}
    elif context.instrument_id in {"F05", "F06"}:
    ...
    payload = {"schema_version": SCHEMA_VERSION, "spec": encode_snapshot_spec(spec), "rows": rows,
               "attendances": records, "population": population, "source_revisions": provenance}
    if anonymous_annual:
        from apps.governance.f05_registry import for_version
        payload.update(contract=for_version(selected.instrument_version.version).CONTRACT,indicator=binding.indicator.code,eligible_count=group_count)
...
def _run_manifest(collection_round, cutoff, identity, prepared):
    ...
    public = list(PublicCollection.objects.filter(binding__collection_round=collection_round).values('binding_id', 'contract_version', 'level', 'programme'))
    if public:
        manifest['intake'] = {'method': 'public_self_selected', 'verified_unique_people': False,
            'denominator': 'aggregate_reference_population',
            'contracts': [{**row, 'binding_id': str(row['binding_id'])} for row in public],
            'limitation': 'Submissions may include repeated participation. Bearer proof does not verify identity.'}
    return manifest
...
def _record_calculation(actor, *, round_id, inputs, cutoff, idempotency_key, dry_run=False, permissions):
    ...
    identity = snapshot_engine({p["payload"].get("contract") for p in prepared} - {None})
...
def validate_run(actor, *, run_id):
    ...
    if run.engine_hash != run.manifest["engine"]["hash"] or snapshot_engine({p.payload.get("contract") for p in run.inputs.all()} - {None})["hash"] != run.engine_hash:
    ...
    for source in sources:
        ...
        if stored is None or stored.source_id != source.pk or stored.result_hash != digest(stored.payload) or replay_snapshot(source.payload) != stored.payload:
...
def replay_snapshot(payload):
    if payload.get('contract'):
        from apps.governance.f05_registry import for_contract
        return for_contract(payload['contract']).replay(payload)
    return replay_input(payload)


def snapshot_engine(contracts=()):
    ...
    if contracts:
        ...
        for contract in sorted(contracts):
            path=Path(for_contract(contract).__file__)
            files[path.name]=hashlib.sha256(path.read_bytes()).hexdigest()
        identity={**identity,'files':files,'hash':digest(files)}
    return identity


def encode_snapshot_spec(spec):
    from apps.governance.f05_registry import MODULES
    if spec.instrument_version in {m.VERSION for m in MODULES}:
        from dataclasses import asdict
        value=asdict(spec);value['question_ids']=list(spec.question_ids)
        return value
