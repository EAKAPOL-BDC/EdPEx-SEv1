...
from .presentation import question_sections
...
def overview(request, scope):
    ...
    selections = RoundInstrument.objects.filter(collection_round__scope=scope,
        instrument_version__instrument__code='F06',survey_profile__isnull=True).select_related('collection_round', 'instrument_version').order_by(
        '-collection_round__created_at', 'pk')
    listing = collection_listing(request, selections, bindings=True)
    return render(request, 'selfassessments/operator_list.html', {'scope': scope, 'listing': listing,
        'can_setup': can_access(request.user, 'round.manage', scope),
        'selections': listing['page']})
...
def collection(request, scope, selected_id):
    ...
    return render(request, 'selfassessments/operator_collection.html', {'scope': scope, 'selected': selected,
        'reason_form': reason, 'can_assign': False,
...
def assign(request, scope, selected_id, member_id):
    ...
    return render(request, 'selfassessments/operator_assign.html', {'scope': scope, 'selected': selected,
        ...
        'question_sections': question_sections(form, english=request.LANGUAGE_CODE == 'en', assignment=True)}, status=code)
...
def result(request, scope, run_id):
    ...
    run = get_object_or_404(CalculationRun.objects.select_related('review_request__decision',
        ...
        pk=run_id, collection_round__scope=scope, stored_source__round_instrument__instrument_version__instrument__code__in=['F01','F02','F03','F04','F05','F06'])
    review = getattr(run, 'review_request', None)
    decision = getattr(review, 'decision', None) if review else None
    ...
    from apps.calculations.admin_review import can_self_review
    admin_self_review = review is not None and request.user.pk in {run.created_by_id, review.requested_by_id} and can_self_review(request.user, scope)
    can_decide = (review is not None and can_review and allowed(request.user, scope, 'result.approve')
                  and (request.user.pk not in {run.created_by_id, review.requested_by_id} or admin_self_review))
    from apps.surveys.models import SurveyProfile
    f04_profile = SurveyProfile.objects.filter(binding=run.stored_source.round_instrument, annual_target__isnull=False).select_related('annual_target__person').first()
    if f04_profile and (f04_profile.annual_target.snapshot.get('evaluatee_user_id') == str(request.user.pk)
                        or f04_profile.annual_target.person.user_id == request.user.pk):
        can_decide = False
    english = request.LANGUAGE_CODE == 'en'
    ...
    selected = run.stored_source.round_instrument
    metadata = run_metadata(run)
    # Current anonymous F06 uses survey routes too; only historical identified
    # self-assessments lack a survey profile. Do not route by instrument code.
    is_self = not hasattr(selected, 'survey_profile')
    result_back = participation_hub(selected) or reverse(
        'operator-collection' if is_self else 'survey-collection', args=[scope.pk, selected.pk])
    prepare_url = reverse('operator-calculate' if is_self else 'survey-calculate', args=[scope.pk, selected.pk])
    return render(request, 'selfassessments/operator_result.html', {'scope': scope,
        'selected': selected, 'run': metadata, 'history': history,
        'result_back': result_back, 'prepare_url': prepare_url,
        'can_prepare': allowed(request.user, scope, 'calculation.run'),
        'can_report': can_review and packet is not None and metadata['status']=='approved' and not metadata['superseded'],
        'admin_self_review': admin_self_review, 'packet': packet, 'packet_error': packet_error, 'submit_form': submit_form, 'decision_form': decision_form,
