...
def detail(request, assignment_id):
    ...
    if request.method == 'POST' and form.is_valid():
        try:
            ...
            response=redirect('self-assessment-detail', assignment_id=assignment_id)
            section=request.POST.get('ui_section','')
            if section in {s['key'] for s in question_sections(form,english=english)}:
                response['Location'] += '#section-'+section
            return response
    ...
    if form.is_bound and form.errors and code == 200:
        code = 422
    return render(request, 'selfassessments/detail.html', {'schema': schema, 'form': form,
        'question_sections': question_sections(form, english=english)}, status=code)
