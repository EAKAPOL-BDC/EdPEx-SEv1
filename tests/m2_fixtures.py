...
def scenario(*, status="closed", extra_f06=False, staff_count=2):
    ...
    population = PopulationSnapshot.objects.create(collection_round=collection_round, definition="Synthetic frozen population",
        counting_unit="per-group defined unit", counts_by_group={"ST1": staff_count, "C5.3": 2}, source=provenance)
    for code, ids in (("ST1", tuple("staff-"+chr(65+i) for i in range(staff_count))), ("C5.3", ("eligibility-org-A", "eligibility-org-B"))):
