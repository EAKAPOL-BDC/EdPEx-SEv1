...
class CalculationSnapshotGuardTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.run = CalculationRun.objects.get(pk=receipt.run_id)

    def test_all_four_tables_reject_sql_update_and_delete(self):
        records = [self.run, self.run.inputs.get(), self.run.results.get(), self.run.requests.get()]
    ...
    def test_cannot_append_input_or_result_to_sealed_run(self):
        for record in (self.run.inputs.get(), self.run.results.get()):
            with self.assertRaises(IntegrityError), transaction.atomic():
                raw_clone(record, series_key="f"*64)

    def test_cannot_forge_a_complete_run_at_insert(self):
        with self.assertRaises(IntegrityError), transaction.atomic():
            raw_clone(self.run, input_hash="f"*64)

    def test_unsealed_run_cannot_commit_even_if_created_by_sql(self):
        manifest = {**self.run.manifest, "synthetic_unsealed": True}
        with self.assertRaises(IntegrityError), transaction.atomic():
            raw_clone(self.run, status="building", sealed_at=None, result_hash="", input_count=0, result_count=0,
    ...
    def test_seal_counts_must_match_actual_inputs_and_results(self):
        manifest = {**self.run.manifest, "synthetic_empty": True}
        with self.assertRaises(IntegrityError), transaction.atomic():
            run_id = raw_clone(self.run, status="building", sealed_at=None, result_hash="", input_count=0, result_count=0,
                               manifest=manifest, input_hash=digest(manifest))
            with connection.cursor() as cursor:
    ...
    def test_receipt_hash_cannot_reference_a_different_request(self):
        with self.assertRaises(IntegrityError), transaction.atomic():
            raw_clone(self.run.requests.get(), idempotency_key="bad-key", request_hash="f"*64)
