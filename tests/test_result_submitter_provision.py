...
class ResultSubmitterProvisionTests(TestCase):
    def setUp(self):
    ...
    def run_command(self,apply=False):
        call_command('provision_result_submitter',scope_id=self.scope.pk,username=self.user.username,
    ...
    def test_preview_does_not_write(self):
        ...
        self.assertFalse(Role.objects.filter(code='result-submitter-v1').exists())
    def test_apply_is_narrow_audited_and_idempotent(self):
        ...
        self.assertTrue(can_access(self.user,'result.submit',self.scope))
        self.assertTrue(can_access(self.user,'calculation.validate',self.scope))
        self.assertFalse(can_access(self.user,'result.submit',self.child))
        self.assertFalse(can_access(self.user,'calculation.validate',self.child))
        for action in ['result.review','result.approve','calculation.source','calculation.run']:
            self.assertFalse(can_access(self.user,action,self.scope))
        self.assertEqual(AuditEvent.objects.filter(action='result.submitter_provisioned').count(),1)
    ...
    def test_mismatched_role_rejected(self):
        Role.objects.create(code='result-submitter-v1',permissions=['result.approve'])
        with self.assertRaises(CommandError):self.run_command(True)
        self.assertEqual(RoleAssignment.objects.count(),1)
